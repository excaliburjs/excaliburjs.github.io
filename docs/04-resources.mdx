...
When building games, you may have assets you want to load from an external file, such as sound, art, or other assets.
...
- `.bmp` are the biggest
- `base64` is useful if you don't want to pay the loading time cost of your images at the expense of larger source download
...
Excalibur supports any audio the browser supports, such as `.wav`, `.mp3`, and `.ogg`. Due to mixed browser support you may wish to specificy a list of
source files and codecs. The order of the files specifies order of preference (we recommend `.mp3` and then `.wav`).
...
If you are developing a game using Excalibur with Webpack, Parcel, or another bundler, these tools typically already come with dev servers for running your game. See [Excalibur project templates](/docs/installation#example-project-templates) for templates you can start from that use these tools.
...
The first path will fail to load because the absolute asset path would now be `/root/assets` and not `/assets`. Use a relative path to load assets _relative_ to the HTML file serving your game.

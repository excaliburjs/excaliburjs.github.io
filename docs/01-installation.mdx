...
There are several ways you can get Excalibur: as standalone packages from `npm` or `nuget`, as raw script files you can download, or by using one of our pre-made [template repos](#github-templates).
...
We recommend that you pin your version of Excalibur to specific version (such as `excalibur@0.24.5`), but you can also get the latest `https://unpkg.com/excalibur@latest`
...
Make sure the path is relative to the current TypeScript (`.ts`) file. You only need to include the reference on your “entrypoint” file. Then simply include `excalibur.min.js` as mentioned above in your HTML page.

...
A scene has a basic lifecycle that directs how it is initialized, updated, and drawn. Once a scene is added to
...
[[Scene.onActivate]] is called when the engine switches to the scene. It may be called more than once during the lifetime of a game if you switch back and forth between scenes. It is useful for taking action before showing the scene. You may use this hook instead of `onInitialize` for anything specific to the context in which the scene was activated.

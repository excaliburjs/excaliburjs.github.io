...
The Excalibur engine uses a straightforward main loop. The engine updates and renders
...
The engine splits the game into two primary responsibilities: updating and drawing. This division is
done to keep your game from trying to draw when its busy calculating logic or trying to calculate logic while its busy drawing.
Trying to update and draw simultaneously can lead to unexpected behavior, so we avoid that scenario in Excalibur.

### Update loop

The first operation that runs is the **Update** loop. Actors and scenes both implement
an overridable/extendable `onPreUpdate` and `onPostUpdate` methods. Use these methods to perform any logic-based operations
in your game for a particular class.

### Draw loop

The next step is the **Draw** loop. A scene iterates through its child actors and
...
you want. However, there always has to be at least one scene and only **one** scene can be
...
In Excalibur, there are option bag constructors available on most types. These support any public property or member, but **methods are not supported.** The API documentation does not provide an exhaustive list of possible properties, but instead provides a list of commonly used properties.
...
Relatedly, you can create a duplicate this way
...
Types that support option bags can have their properties bulk assigned using the assign method.
...
You can customize the options or provide more options for your game by extending [[Engine]].
...
You can also emit any other kind of event for your game by using a custom
...
Excalibur does not provide any out-of-the-box way to manage game state, but you can typically either use class properties or introduce something more sophisticated like a [state machine](https://github.com/davidkpiano/xstate).

The benefit of a state machine is that state can be separated from the actions an actor may take and you can then _save_ and _load_ state more easily to enable save game management. For example, you could choose to have a global game state that you can serialize and deserialize.

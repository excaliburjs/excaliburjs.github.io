...
[[Actor|Actors]] are prebuilt Excalibur [[Entity|Entities]] that come preconfigured for most applications.
...
If you don't want these builtins and you want to build only what you want, read me about entities [here](/docs/entities)
...
For more complicated games, it is common to extend the `Actor` class.
This gives you much greater control and encapsulates logic for that
actor. Custom actors are optional, but they're nice for encapsulating specific behavior for different types of actors.
...
An actor has a standardized lifecycle that dictates how it is initialized, updated, and drawn. Once an actor is part of a
...
This is the recommended way to manage startup logic for an actor. We do _not_ recommend putting logic in the constructor,
because you will incur the cost of initialization before an actor is ready to be updated in the game.
